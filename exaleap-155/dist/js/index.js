...
var architecture = {
    ...
    makeBuilding(g3d, firstFloor, lastFloor, maxFloorIndex, callBacks, defXZ) {
        let firstFloorVector3 = [], lastFloorVector3  = [], dm3d = g3d.dm();
        const { getColor = () => '222,222,222', getOpacity = () => 1, getTall = () => 1, getThickness = () => 0.5, floorCall = () => {} } = callBacks;
        const Vector3 = ht.Math.Vector3, floorPointLength = firstFloor.getPoints().length, segments = firstFloor.getSegments();
        ...
        if(maxFloorIndex === 1) {
            let points = [], elevation = firstFloorVector3[0].y;
            ...
            for(let i = 0; i < maxFloorIndex; i++) {
                ...
                let points = [], elevation = curFloorPoints[0].y;
    ...
    makeDecShape(g3d, firstFloor, lastFloor, pointIndexMap, callBacks) {
        ...
        let firstFloorWorldPoints = [], lastFloorWorldPoints = [], dm3d = g3d.dm();
...
var common = { notifyCb, showBuildByType, executeBtnAction, startAnimate };
...
class Build {
    ...
    init3dNodes() {
        ...
        all.elevators.forEach((elevator) => {
            let splitDisplayName = elevator.getDisplayName().split('-'),
                { beginIndex, endIndex } = pTools.getBeginAndEndFloorIndex(4, splitDisplayName[1], splitDisplayName[2]);
    ...
    makeBuilding() {
        ...
        first1 = dm3d.getDataByTag('first1'),
        ...
        architecture.makeBuilding(g3d, first3, last3, 5, {
            getColor: (floorNum) => {
            ...
            getOpacity: (floorNum) => {
                ...
                if (floorNum >= 2 && floorNum <= 4) return 0.6;
                return 0.85;
            },
            getTall: (floorNum) => 14,
            getThickness: (floorNum) => 6,
            floorCall: (floorNum, floorNode, wireNode) => {
                const floorNums = ['1F', '2F', '3F', '4F', '5F'];
                floorNode.setDisplayName('楼层地板');
                floorNode.a('floorNum', floorNums[floorNum - 1]);
                floorNode.a('floorType', 'business');
                business.floorNodes.push(floorNode);
        ...
        architecture.makeBuilding(g3d, first4, last4, 9, {
            getColor: (floorNum) => {
                if (floorNum === 1) return '194, 133, 43';
                if (floorNum === 2) return '255, 184, 77';
                return '111, 166, 222'
            },
            getOpacity: (floorNum) => {
                if (floorNum === 1) return 0.6;
        ...
        architecture.makeBuilding(g3d, first5, last5, 6, {
            getColor: (floorNum) => '53, 153, 219',
            getOpacity: (floorNum) => 0.8,
            getTall: (floorNum) => 14,
            getThickness: (floorNum) => 6,
            floorCall: (floorNum, floorNode, wireNode) => {
                const floorNums = ['17F', '18F', '19F', '20F', '21F', '22F'];
                floorNode.setDisplayName('楼层地板');
                floorNode.a('floorNum', floorNums[floorNum - 1]);
                floorNode.a('floorType', 'office');
                office.floorNodes.push(floorNode);
